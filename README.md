...
```text
FastAPI 路由 -> 文档上传 -> 文本切分 -> embeddinggemma 向量 -> 检索 -> 可选择的本地聊天模型 -> 来源约束回答 -> SQLite 笔记 -> 相关来源
```
...
笔记数据默认保存到 `data/processed/learninghub_notes.sqlite`，页面刷新或 FastAPI 重启后仍然存在；该文件属于个人数据，默认不会提交到 GitHub。
...
| 4 | DataPilot 数据分析 Agent | 阶段 11 完成，文档收尾中 | 面向业务数据的本地自然语言分析助手 |
...
5. 如果需要带走个人笔记，单独复制 `data/processed/learninghub_notes.sqlite`，该文件不会提交到 GitHub。
6. 按照 `projects/` 中的任务卡继续学习。
7. 如果希望 Codex 保持相同的工作方式，将 `codex-skills/ai-career-portfolio-coach/` 复制到 `~/.codex/skills/`。
