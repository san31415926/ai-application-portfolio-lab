...
- 工作经历：
- 证书：
- 已验证成果：`projects/02-rag-fastapi-service` 使用 Ollama 的 `embeddinggemma:300m` 生成 768 维检索向量，并使用可选择的本地聊天模型生成有来源约束的中文回答，同时保留稀疏检索和提取式回答降级。项目包含 227 份 Markdown 教程、205 条可搜索学习笔记、SQLite 持久化、防抖自动保存、来源引用、低置信度拒答、批量建索引、笔记工作流、模型发现、OpenAPI 文档、qwen2.5:3b 写作辅助、SSE 流式问答和 17 项通过的 API 测试。使用 `qwen2.5:3b` 回答了 C 语言问题并生成了 Embedding 笔记摘要，同时正确拒绝了无关的火星条约问题。早期原型已成功读取示例岗位 JD PDF，本地 JD 知识库可以索引默认文档，SQLite 模拟数据库包含 5 个岗位和 16 项技能，8 项测试通过。
- 链接：
