...
# 2026-07-31：DataPilot 增加客户数据中心并清理内部文案

- 删除 Streamlit 页面底部“当前阶段验收”和“下一阶段将补充失败案例记录和演示截图”等内部开发信息，同时去掉标题副标题中的阶段编号。
- 新增 `src/practice_data.py` 和 `scripts/generate_practice_data.py`，生成 10 组固定种子的中文合成练习数据，每组同时提供 CSV 和 XLSX，共 20 个文件。
- 页面新增“练习数据中心”，展示 10 个主题的数据量、字段数、适合练习场景和建议问题；用户可以预览、下载两种格式，或切换到练习数据直接进入分析流程。
- 验证：新增练习数据生成、CSV/XLSX 一致性和页面练习模式测试；完整测试集 76 项通过，`compileall` 和 `git diff --check` 通过；外置 Edge 实测首页、10 个数据主题、练习预览和 CSV/XLSX 下载按钮均可见。
- 长期规则：客户页面不展示开发阶段、后续计划、待补充事项、失败案例文档或截图说明；这些内容只能保留在项目文档中。
