...
- **处理**：文档解析与清洗后切成 chunks；优先调用 Ollama `embeddinggemma:300m` 生成向量并按余弦相似度检索；命中阈值后将来源片段交给默认或用户选择的本地聊天模型生成回答。
...
client
  -> FastAPI routers
      -> knowledge router: upload/list/chunks/delete
      -> chat router: query
      -> notes router: notes/search/related/assist/review
  -> RagService
      -> document loader
      -> text splitter
      -> Ollama/local embedding retriever
      -> local model registry and selectable answer generator
      -> answer generator with source citations
  -> NoteService
      -> note CRUD
      -> note search
      -> related source retrieval
      -> deterministic writing assistant
...
Select a specific local answer model:
...
Tests default to the sparse and extractive fallback so they can run without requiring Ollama. 当前 API 测试共 7 项；要验证本地 embedding 和聊天模型路径，请保持 Ollama 运行并使用上面的示例 API 调用。
