...
- `/api/v1/chat/query/stream` 以 SSE 流式返回来源、答案片段和完成状态，页面使用该接口逐步显示回答
- 页面中的来源文件名可以点击，打开完整文档切片并高亮当前命中的原文，方便用户核验回答依据
- 支持查看文档列表、文档 chunks、知识库统计、删除文档、加载示例资料
- 支持发现本机已安装的 Ollama 生成模型，并在页面中选择本次问答使用的模型
- 笔记管理：新建、编辑、删除、搜索、标签和分类
- 笔记使用 SQLite 持久化，服务重启后自动恢复，示例笔记导入保持幂等
- 编辑已有笔记时使用 900ms 防抖自动保存，页面停止输入后通过 `PATCH` 接口同步到后台
...
- **处理**：文档解析与清洗后进行文本切分；优先调用 Ollama `embeddinggemma:300m` 生成向量并按余弦相似度检索；命中阈值后将来源片段交给默认或用户选择的本地聊天模型生成回答；笔记通过 SQLite 完成持久化 CRUD 和回顾状态保存。
- **输出**：中文回答、来源文件、切片序号、相关度、回答模型和拒答状态；笔记还可以关联知识库来源。
- **难点**：上下文长度有限，不能把全部文档直接交给模型；检索相关度不足时要在生成前拒答；模型不可用时要保留可演示的降级路径。
- **改进**：当前向量索引仍保存在内存中，后续升级为 Chroma；笔记已经使用 SQLite 持久化，并计划增加检索评估集、回答质量评估和部署配置。
...
- 当前向量暂存在内存中，适合简历项目 MVP；笔记数据已保存到 `data/processed/learninghub_notes.sqlite`，后续升级路径是将向量持久化到 Chroma，并补充检索评估样例。
- 可通过 `NOTE_DB_PATH` 修改 SQLite 文件位置。该数据库属于本地个人数据，默认被 `.gitignore` 忽略；换电脑时可单独复制数据库文件，或重新导入示例笔记。
...
测试默认使用稀疏检索和提取式回答降级，因此不依赖 Ollama 也能运行。当前 API 测试共 17 项，包含 SQLite 重启恢复验证；要验证本地 Embedding、聊天模型、写作辅助和流式问答路径，请保持 Ollama 运行并使用上面的示例 API 调用。
...
- 使用 SQLite 持久化笔记正文、标签、分类和间隔复习状态，服务重启时恢复数据，并通过幂等元数据避免示例笔记重复导入。
...
- 实现低置信度拒答、生成失败降级、批量向量索引、qwen2.5:3b 写作辅助和笔记关联知识库机制，通过测试覆盖静态工作台、示例入库、文档上传、查询命中、无证据拒答、笔记关联和本地写作辅助场景。

## 下一步

- 将内存向量索引升级为 Chroma 向量库
- 补充检索评估样例和阈值调优记录
- 增加本地 LLM 写作辅助的流式输出和质量评估
- 增加用户隔离、登录和会话历史
